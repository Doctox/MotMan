...
try {
  ...
  assert.ok(catalog.version >= 16)
  assert.equal(catalog.grids.length, 25)
  ...
  assert.equal(catalog.grids.reduce((count, grid) => count + grid.words.filter(word => word.image).length, 0), 117)
